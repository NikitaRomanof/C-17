...
std::string Model::rebuildString() {
  ...
  bool error = true;
  for (; i < m_inputString.size();) {
    if (m_inputString[i] == 'm') {
      if (m_inputString[i + 1] == 'o' && m_inputString[i + 2] == 'd') {
        newStr[n] = 'm';
        helperRebuild(i, n, pos, error, 3, true);
      } else {
        throw std::invalid_argument("Bad symbol");
      }
    } else if (m_inputString[i] == 's') {
      if (m_inputString[i + 1] == 'i' && m_inputString[i + 2] == 'n') {
        newStr[n] = 's';
        helperRebuild(i, n, pos, error, 3, false);
      } else if (m_inputString[i + 1] == 'q' && m_inputString[i + 2] == 'r' &&
                 m_inputString[i + 3] == 't') {
        newStr[n] = 'q';
        helperRebuild(i, n, pos, error, 4, false);
      }
      errorFoo(m_inputString, pos, error);
    } else if (m_inputString[i] == 'c') {
      if (m_inputString[i + 1] == 'o' && m_inputString[i + 2] == 's') {
        newStr[n] = 'c';
        helperRebuild(i, n, pos, error, 3, false);
      }
      errorFoo(m_inputString, pos, error);
    } else if (m_inputString[i] == 't') {
      if (m_inputString[i + 1] == 'a' && m_inputString[i + 2] == 'n') {
        newStr[n] = 't';
        helperRebuild(i, n, pos, error, 3, false);
      }
      errorFoo(m_inputString, pos, error);
    } else if (m_inputString[i] == 'a') {
      if (m_inputString[i + 1] == 't' && m_inputString[i + 2] == 'a' &&
          m_inputString[i + 3] == 'n') {
        newStr[n] = 'n';
        helperRebuild(i, n, pos, error, 4, false);
      } else if (m_inputString[i + 1] == 's' && m_inputString[i + 2] == 'i' &&
                 m_inputString[i + 3] == 'n') {
        newStr[n] = 'i';
        helperRebuild(i, n, pos, error, 4, false);
      } else if (m_inputString[i + 1] == 'c' && m_inputString[i + 2] == 'o' &&
                 m_inputString[i + 3] == 's') {
        newStr[n] = 'o';
        helperRebuild(i, n, pos, error, 4, false);
      }
      errorFoo(m_inputString, pos, error);
    } else if (m_inputString[i] == 'l') {
      if (m_inputString[i + 1] == 'n') {
        newStr[n] = 'l';
        helperRebuild(i, n, pos, error, 2, false);
      } else if (m_inputString[i + 1] == 'o' && m_inputString[i + 2] == 'g') {
        newStr[n] = 'g';
        helperRebuild(i, n, pos, error, 3, false);
      }
      errorFoo(m_inputString, pos, error);
    } else if (m_inputString[i] == 'n' || m_inputString[i] == 'q' ||
               m_inputString[i] == 'i' || m_inputString[i] == 'g' ||
               m_inputString[i] == 'o') {
      throw std::invalid_argument("Bad symbol");
    } else {
      newStr[n] = m_inputString[i];
...
double Model::start() {
  ...
  checkStr(str);
  checkDot(str);
  m_input = new std::istringstream(str);
  double tmp = 0.0;
  while (m_input) {
    get_token();
    if (curr_tok == end) {
...
std::vector<double> Model::credit_annuity(double sum_credit, double time_credit,
                                          double per) {
...
std::vector<double> Model::deposit_calculator(std::map<double, double> r1,
  ...
  if (kap == 0) {
    ...
    for (int i = 1; i <= tern; i++) {
      ...
      all_sum = all_sum - get_wi_list(w1, i, all_sum);
      if (i % period_s == 0) {
        all_interest += (interest / 12.0) * period_s * (all_sum + all_interest);
        p = i;
      } else if (i == tern) {
        int z = i - p;
